...
public class WelcomeRestService {
     ...
     * <p/>
     * By default the URL will point to 'http://localhost:8080/Server-server' which should hit your
     ...
     *                  'http://localhost:8080/Server-server'
     */
...
//                serverUrl + "/game?name={name}", WelcomeMessage.class, values);
        WelcomeMessage welcomeMessage = restTemplate.postForObject(serverUrl+"/game?name1={name1}", name, WelcomeMessage.class, values);
    ...
    public Boolean LogIn(String name,String pass)
    {
        ...
        Boolean mess = restTemplate.getForObject(serverUrl+"/login?name={name}&pass={pass}", Boolean.class, values);
        return mess;
    }
}
